        ...
        for dl, bs, epoch in [(1000,16,30),(3000,16,10),(6000,16,5),(10000,16,3)]:
            for lr in [2e-4]:
                ...
                for eval_dataset in evalsets:
                    ...
                    os.system(f'CUDA_VISIBLE_DEVICES={gpu} python {eval_file} --model {model_name} --adapter LoRA --dataset {eval_dataset} --base_model {base_model} --lora_weights ./trained_models/{model}_{dataset}_dl{dl}bs{bs}epoch{epoch}_slora_r{r}_lr{lr}_seed{seed}')
